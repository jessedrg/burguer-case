...
let S3CoreModule = S3CoreModule_1 = class S3CoreModule {
    static forRoot(options, connection) {
        const s3OptionsProvider = {
            provide: s3_utils_1.getS3OptionsToken(connection),
            useValue: options,
        };
        const s3ConnectionProvider = {
            provide: s3_utils_1.getS3ConnectionToken(connection),
            useValue: s3_utils_1.createS3Connection(options),
        };
        return {
    ...
    static forRootAsync(options, connection) {
        const s3ConnectionProvider = {
            provide: s3_utils_1.getS3ConnectionToken(connection),
            useFactory(options) {
                return s3_utils_1.createS3Connection(options);
            },
            inject: [s3_utils_1.getS3OptionsToken(connection)],
        };
        return {
            module: S3CoreModule_1,
